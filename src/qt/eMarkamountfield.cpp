...
eMarkAmountField::eMarkAmountField(QWidget *parent):
...
void eMarkAmountField::setText(const QString &text)
{
    if (text.isEmpty())
        amount->clear();
    else
...
void eMarkAmountField::clear()
{
    amount->clear();
    unit->setCurrentIndex(0);
}

bool eMarkAmountField::validate()
{
    bool valid = true;
    if (amount->value() == 0.0)
        valid = false;
...
void eMarkAmountField::setValid(bool valid)
{
    if (valid)
...
QString eMarkAmountField::text() const
{
...
bool eMarkAmountField::eventFilter(QObject *object, QEvent *event)
{
...
QWidget *eMarkAmountField::setupTabChain(QWidget *prev)
{
...
void eMarkAmountField::setDisplayUnit(int newUnit)
{
